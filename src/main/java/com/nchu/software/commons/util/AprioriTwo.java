 ...
 * @description
 * @create 2019-01-14 23:08
 */
@Data
public class AprioriTwo {
    public  int times=0;//迭代次数
    double MIN_SUPPROT = 0.1;//最小支持度百分比
    double MIN_CONFIDENCE=0.6;//最小置信度
    boolean endTag = false;//循环状态，迭代标识
    List<List<String>> record = new ArrayList<>();//数据集
    List<List<String>> frequentItemset=new ArrayList<>();//存储所有的频繁项集
    List<MyMap> map = new ArrayList();//存放频繁项集和对应的支持度技术

    public  List<List<String>> calcApriori(){
        ...
        List<List<String>> results =  AssociationRulesMining();//挖掘关联规则
        return results;
    }
    ...
    public List<List<String>> AssociationRulesMining()//关联规则挖掘
    {
        ...
        for(int i=0;i<frequentItemset.size();i++)
        {
            ...
            if(tem.size()>1 && (tem.get(tem.size() - 1).equals("18") || tem.get(tem.size() - 1).equals("19") || tem.get(tem.size() - 1).equals("20"))) {
                ...
                for (int j = 0; j < AllSubset.size(); j++) {
                    ...
                    if (s2.size() == 1 && (s2.get(0).equals("18") || s2.get(0).equals("18") || s2.get(0).equals("18"))){
    ...
    private List<List<String>> getSupprotedItemset(List<List<String>> CandidateItemset) { //对所有的商品进行支持度计数
        ...
        for (int i = 0; i < CandidateItemset.size(); i++){
            ...
            System.out.println("第 " + CandidateItemset.get(i).toString() + " 个" + "count=" + count);
    ...
    public List<List<String>> findFirstCandidate() {
        ...
        HashSet<String> hs  = new HashSet<>();//新建一个hash表，存放所有的不同的一维数据
        ...
        while(itr.hasNext()){
            ...
            String Item = (String) itr.next();
            tempList.add(Item);   //将每一种规则存放到一个List<String>中
            tableList.add(tempList);//所有的list<String>存放到一个大的list中
        }
        return tableList;//返回所有的规则
    }
}
