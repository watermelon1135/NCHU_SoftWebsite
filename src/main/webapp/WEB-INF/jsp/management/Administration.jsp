			...
			<div class="row">
				...
				<div class="col-md-10" style="margin-top: 30px;padding: 0px 50px 0px 30px;">
					<div style="height:50px;width:100%">
						...
						<div style="float:right;width:20%">
							<form action="/admin/search" method="get">
								<input type="text" placeholder="输入关键字进行查找" class="form-control" style="display: inline;width: 160px" value="${keyword}" name="keyword">
					...
					<div class="nr-content">
						<table class="table">
							...
							<c:forEach items="${pagecontexts}" var="page" varStatus="index">
								<tr>
									...
									<td><button class="btn" onclick="jupm('/admin/updatecontent/${page.pageContextId}')" data-toggle="" data-target="l">修改</button></td>
									<td><button class="btn" onclick="return addRecycle(${page.pageContextId})" data-toggle="modal" data-target="#myModal2">放入回收站</button></td>
						...
						<form action="/search" method="get">
							<input type="text" placeholder="输入关键字进行查找" value="${keyword}" name="keyword">
							<input type="submit" value="查询">
						</form>
						<c:forEach items="${pagecontexts}" var="page">
							<li>
								<a href="/item/${page.pageContextId}" title="时间:<fmt:formatDate value="${page.uploadTime}" pattern="yyyy年MM月dd日"/>   标题:${page.title}" target="_blank">${page.title}</a>
									&lt;%&ndash;<span class="pull-right glyphicon glyphicon-chevron-down"></span>&ndash;%&gt;
								&nbsp;&nbsp;<a href="/updatecontent/${page.pageContextId}">修改</a>
								&nbsp;&nbsp;<a href="#" onclick="return addRecycle(${page.pageContextId})">放入回收站</a>
								&nbsp;&nbsp;<a href="" onclick="deletePageContent(${page.pageContextId})">彻底删除</a>
				...
				<c:if test="${not empty pagecontexts}">
					<div style="width:100%;text-align:center">
						<ul class="pagination">
							<li><a href="<c:if test="${pages.pageNum!=1}">${curr}?keyword=${keyword}&pagenum=${pages.pageNum-1}</c:if>"> &laquo;</a></li>
							...
							<li class="<c:if test="${i==pages.pageNum}">active</c:if> "><a href="${curr}?keyword=${keyword}&pagenum=${i}">${i}</a></li>
							</c:forEach>
							<c:if test="${pages.pageNumAll>pagesEnd}">
							<li><a href="${curr}?keyword=${keyword}&pagenum=${pages.pageNum+5}">...</a></li>
							</c:if>
							<li><a href=" <c:if test="${pages.pageNum!=pages.pageNumAll}">${curr}?keyword=${keyword}&pagenum=${pages.pageNum+1}</c:if>">&raquo;</a></li>
					</div>
				</c:if>
			</div>
		</div>
